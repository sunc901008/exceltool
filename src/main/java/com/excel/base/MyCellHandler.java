...
public class MyCellHandler extends DefaultHandler {
    private static final Logger logger = Logger.getLogger(MyCellHandler.class);
    ...
    private List<CellContainer> record = new ArrayList<>();
    private List<List<CellContainer>> rows = new ArrayList<>();
    ...
    MyCellHandler(ReadOnlySharedStringsTable strings, StylesTable styles) {
    ...
    public void endElement(String uri, String localName, String name) {
        if ("v".equals(name)) {
            ...
            while (addSpace > 1) {
                record.add(new CellContainer(CellContainer.NONE, ""));
                addSpace--;
            }
            thisStr = thisStr.replaceAll("(\r\n|\r|\n|\n\r)", " ");
            record.add(new CellContainer(type, thisStr));
            if (thisColumn > -1) {
    ...
    List<List<CellContainer>> getRows() {
        return rows;
    }
